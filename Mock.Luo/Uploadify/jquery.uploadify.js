...
(function ($) {
        ...
        init: function (options, swfUploadOptions) {
            ...
            return this.each(function () {
                ...
                if (flashInstalled) {
                    ...
                    $button.html('<b class="nui-ico2"></b><span class="uploadify-button-text">' + settings.buttonText + '</span>')
					.css({
					    'height': settings.height + 'px',
                ...
                if (args[0]) {
                    ...
                    if (args[0] == '*') {
                        ...
                        $('#' + settings.queueID).find('.uploadify-queue-item').each(function () {
                            ...
                            $(this).find('.nui-txt-suc').removeClass('data').html(' - 删除中...');
                        ...
                        for (var n = 0; n < args.length; n++) {
                            ...
                            $('#' + args[n]).find('.nui-txt-suc').removeClass('nui-txt-suc').html(' - 删除中...');
                            $('#' + args[n]).find('.uploadify-progress-bar').remove();
                            $('#' + args[n]).delay(100 + 100 * n).fadeOut(500, function () {
                                $(this).remove();
                            });
                        }
                    }
                    ...
                    $item.find('.nui-txt-suc').removeClass('nui-txt-suc').html(' - 删除中...');
    ...
    var handlers = {
        ...
        onDialogOpen: function () {
            ...
            this.queueData.errorMsg = '有些文件没有被列入排队:';
        ...
        onSelect: function (file) {
            ...
            var fileTitle = file.name.replace(/\s+/g, "");
            ...
            filetype = "nui-ico-file32-" + filetype;
            ...
            if (settings.itemTemplate == false) {
                settings.itemTemplate = '<div id="${fileID}" class="uploadify-queue-item">\
                    <b id="_mail_icon_47_239" class="js-component-icon nui-ico nui-ico-file32 ${filetype}  "></b>\
                    <span class="fileName" title="${fileTitle}">${fileName}</span>\
					<div class="cancel">\
						<a href="javascript:$(\'#${instanceID}\').uploadify(\'cancel\', \'${fileID}\')" class="nui-txt-link">删除</a>\
					</div>\
                    <div class="data"><span class="fileSize">${fileSize}</span><span class="nui-txt-suc"></span></div>\
					<div class="uploadify-progress">\
						<div class="uploadify-progress-bar"><!--Progress Bar-->\
                           <input type="hidden" name="hiddenUrl"/>\
                        </div>\
					</div>\
				</div>';
            }
        ...
        onUploadError: function (file, errorCode, errorMsg) {
            ...
            if ($.inArray('onUploadError', settings.overrideEvents) < 0) {
                ...
                if (errorCode != SWFUpload.UPLOAD_ERROR.SPECIFIED_FILE_ID_NOT_FOUND && file.status != SWFUpload.FILE_STATUS.COMPLETE) {
                    $('#' + file.id).find('.nui-txt-suc').html(' - ' + errorString);
                }
            }
        ...
        onUploadProgress: function (file, fileBytesLoaded, fileTotalBytes) {
            ...
            if ($.inArray('onUploadProgress', settings.overrideEvents) < 0) {
                if (settings.progressData == 'percentage') {
                    if (percentage == 100) {
                        $('#' + file.id).find('.nui-txt-suc').html('上传完成');
                        //$('#' + file.id).find('.uploadify-progress').css('display', 'none');
                    }
                    else {
                        $('#' + file.id).find('.nui-txt-suc').html(' - ' + percentage + '%');
                    }
                } else if (settings.progressData == 'speed' && lapsedTime > 500) {
                    $('#' + file.id).find('.nui-txt-suc').html(' - ' + this.queueData.averageSpeed + suffix);
                }
        ...
        onUploadSuccess: function (file, data, response) {
            ...
            if ($.inArray('onUploadSuccess', settings.overrideEvents) < 0) {
                //$('#' + file.id).find('.nui-txt-suc').append('上传完成');
            }
...
//绑定上传控件
$.fn.bindUploadify = function (options) {
    var defaults = {
        hiddenUrl: [],
        fileName: []
    };
    ...
    for (var i = 0; i < options.hiddenUrl.length; i++) {
        var fileName = options.fileName[i].replace(/\s+/g, "");
        var fileTitle = fileName;//文件全称
        var url = options.hiddenUrl[i];//实际地址

        id = url.substring(url.lastIndexOf('/')+1, url.length);//文件名是唯一的，取出文件名作为id值
        console.log(id);
        var fileSize = options.fileSize[i];

        if (fileName.length > 25) {
            fileName = fileName.substr(0, 25) + '...';
        }

        var filetype = fileTitle.split('.').pop().toLowerCase();//得到后缀

        filetype = "nui-ico-file32-" + filetype;

        var contentHtml = '<div id=' + id + ' class="uploadify-queue-item">\
                    <b class="js-component-icon nui-ico nui-ico-file32 '+ filetype + ' "></b>\
                    <span class="fileName" title=' + fileTitle + '>' + fileName + '</span>\
					<div class="cancel">\
						<a href="javascript:void(0)" onclick="RemoveFile(\'' + id + '\')" class="nui-txt-link">删除</a>\
					</div>\
                    <div class="data"><span class="fileSize">'+fileSize+'</span><span class="nui-txt-suc">上传完成</span></div>\
					<div class="uploadify-progress">\
						<div class="uploadify-progress-bar" style="width:100%;">\
                           <input type="hidden" name="hiddenUrl"  value="'+ url + '"/>\
                        </div>\
					</div>\
				</div>';
        var inputID = $(this).attr('id');
        $('#' + inputID + '-queue').append(contentHtml);
    }
}

function RemoveFile(id) {
    $('div[id="' + id + '"]').find('.nui-txt-suc').removeClass('nui-txt-suc').html(' - 删除中...');
    $('div[id="' + id + '"]').find('.uploadify-progress-bar').remove();
    $('div[id="' + id + '"]').delay(100).fadeOut(500, function () {
        $(this).remove();
    });
}